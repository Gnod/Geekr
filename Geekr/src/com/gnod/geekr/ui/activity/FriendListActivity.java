...
public class FriendListActivity extends BaseActivity {
				...
				if(firstVisibleItem + visibleItemCount >= totalItemCount && 
					...
					if(isAllLoaded == false && !mFooter.isLoading()) {
						...
						mFetcher.fetchFollows(mUserInfoModel.userID, mUserInfoModel.nickName, 
								type == TYPE_FOLLOWERS? 0 : 1,30, mList.size() + 1, onFetchListener);
					}
	...
	private void bindView() {
		...
		if(extras == null)
			finish();
		mUserInfoModel = (UserInfoModel) extras.getSerializable("UserInfoModel");
	...
	private BaseAdapter userAdapter = new BaseAdapter() {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if(convertView == null) {
				...
				convertView = LayoutInflater.from(FriendListActivity.this).inflate(R.layout.listitem_users, null);
				itemView.imageAvatar = (AvatarView) convertView.findViewById(R.id.image_users_item_avatar);
				itemView.textName = (TextView) convertView.findViewById(R.id.text_users_item_name);
			...
			mDrawableMgr.loadBitmap(user.iconURL, itemView.imageAvatar, true);
