...
public class CommentsActivity extends BaseActivity {
		...
		mListView.setOnScrollListener(new AbsListView.OnScrollListener() {
			...
			public void onScroll(AbsListView view, int firstVisibleItem,
				...
				if(firstVisibleItem + visibleItemCount >= totalItemCount && 
					...
					if(isAllLoaded == false && mRefresh.getActionView() == null) {
						setRefreshing(true);
	...
	private void bindView() {
		...
		getSupportActionBar().setListNavigationCallbacks(mSpinAdapter, new OnNavigationListener() {
			@Override
		...
		if(extras != null){
			...
			if(mSelectedType == 2)
				...
				WeiboBaseTool.getInstance().resetUnRead(UNREAD_TYPE.MENTION_CMT, null);
		}
	}
			...
			case NoticeFetcher.FETCH_SUCCEED_MORE:
				...
				if(resultList.size() > 1){
					if(state == NoticeFetcher.FETCH_SUCCEED_NEWS){
						...
						mBottomToastView.show("更新" + resultList.size() + "条新消息", 
								ToastColor.BLUE);
	...
	private BaseAdapter listAdapter = new BaseAdapter() {
		...
		public View getView(int position, View view, ViewGroup parent) {
			...
			if(view == null){
				...
				item = new ItemView();
				item.imageAvatar = (AvatarView) view.findViewById(R.id.layout_met_item_avatar);
				item.imageVerified = (ImageView) view.findViewById(R.id.layout_met_item_verified_image);
				item.textName = (TextView) view.findViewById(R.id.layout_met_item_name);
				item.textContent = (TextView) view.findViewById(R.id.layout_met_item_content); 
				item.textSource = (TextView)view.findViewById(R.id.layout_met_item_from);
				item.layoutRetweet = view.findViewById(R.id.layout_met_item_retweet);
				item.textRetweetContent = (TextView) view.findViewById(R.id.layout_met_item_retweet_content);
				item.textTime = (TextView) view.findViewById(R.id.layout_met_item_time);
			...
			mDrawableMgr.loadBitmap(userInfo.iconURL, item.imageAvatar, true);
			item.textName.setText(userInfo.nickName);
			if(userInfo.verifiedType == 3 || 
					userInfo.verifiedType == 2 || 
					userInfo.verifiedType == 7){
				item.imageVerified.setVisibility(View.VISIBLE);
				item.imageVerified.setImageResource(R.drawable.ic_verified_blue);
			...
			if(model.replyComment == null && model.status == null) {
				...
				if(model.replyComment.userInfo != null);
					content = "回复 @" + model.replyComment.userInfo.nickName + " 的评论: ";
			...
			item.textTime.setText(
					DateUtils.getMagicTime(model.time));
